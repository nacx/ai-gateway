...
const defaultMCPBackendResourcePrefix = "default-mcp-backend__"
...
func TestMCP(t *testing.T) {
	...
	env := requireNewMCPEnv(t, true, 1200*time.Second)
	...
	env = requireNewMCPEnv(t, false, 1200*time.Second)
	for _, tc := range tests {
	...
	env = requireNewMCPEnv(t, false, 2*time.Second)
	for _, tc := range tests {
