...
func requireNewMCPEnv(t *testing.T, forceJSONResponse bool, writeTimeout time.Duration) *mcpEnv {
	...
	m.extProcMetricsURL = fmt.Sprintf("http://localhost:%d/metrics", env.ExtProcMetricsPort())
	m.baseURL = fmt.Sprintf("http://localhost:%d/mcp", env.EnvoyListenerPort())
