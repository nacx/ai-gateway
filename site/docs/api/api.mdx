...
<ApiField
  ...
  type="string array"
  required="true"
  description="Include is a list of strings to include. If specified, only the strings in this list are included."
/><ApiField
  name="includeRegex"
  type="string array"
  required="true"
  description="IncludeRegex is a list of RE2-compatible regular expressions that, when matched, include the string."
/>
...
/><ApiField
  name="toolSelector"
  type="[MCPNameFilter](#mcpnamefilter)"
  required="true"
  description="ToolSelector filters the tools exposed by this MCP server.<br />Supports exact matches via include lists and RE2-compatible regular expressions via includeRegex.<br />If not specified, all tools from the MCP server are exposed."
/><ApiField
  name="securityPolicy"
...
/><ApiField
  ...
  required="true"
  description="BackendRefs is a list of backend references to the MCP servers.<br />These MCP servers will be aggregated and exposed as a single MCP endpoint to the clients.<br />From the client's perspective, they only need to configure a single MCP server URL, e.g. `https://api.example.com/mcp`,<br />and the Envoy AI Gateway will route the requests to the appropriate MCP server based on the requests."
/><ApiField
  name="securityPolicy"
