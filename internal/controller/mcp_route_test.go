...
func TestMCPRouteController_Reconcile(t *testing.T) {
	...
	for _i, expName := range []gwapiv1.ObjectName{"svc-a", "svc-b"} {
		...
		require.Len(t, httpRoute.Spec.Rules[i].Matches[0].Headers, 1)
		require.Equal(t, internalapi.MCPBackendHeader, string(httpRoute.Spec.Rules[i].Matches[0].Headers[0].Name))
	}
