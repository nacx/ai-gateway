...
type (
	...
	mcpProxyConfig struct {
		...
		routes              map[string]map[filterapi.MCPBackendName]filterapi.MCPBackend // route name -> backends of that route.
		backends            map[filterapi.MCPBackendName]filterapi.MCPBackend            // all backends referenced by any route.
		toolFilters         map[filterapi.MCPBackendName]*toolFilter                     // precomputed tool filters for each backend.
	}
...
func (m *MCPProxy) LoadConfig(_ context.Context, config *filterapi.Config) error {
	...
	newConfig.routes = make(map[string]map[filterapi.MCPBackendName]filterapi.MCPBackend, len(mcpConfig.Routes))

	for _, route := range mcpConfig.Routes {
		newConfig.routes[route.Name] = make(map[filterapi.MCPBackendName]filterapi.MCPBackend, len(route.Backends))
		for _, backend := range route.Backends {
			newConfig.routes[route.Name][backend.Name] = backend
		}
	}

	// Also build a map of all backends by name.
	newConfig.backends = make(map[filterapi.MCPBackendName]filterapi.MCPBackend)
	for _, backends := range newConfig.routes {
		for name, backend := range backends {
			newConfig.backends[name] = backend
		}
	}

	// Precompute tool filters for each backend.
	newConfig.toolFilters = make(map[filterapi.MCPBackendName]*toolFilter, len(newConfig.backends))
	for name, backend := range newConfig.backends {
		tf := &toolFilter{
			include: make(map[string]struct{}),
		}
		if backend.ToolSelector != nil {
			for _, tool := range backend.ToolSelector.Include {
				tf.include[tool] = struct{}{}
			}
			for _, expr := range backend.ToolSelector.IncludeRegex {
				re, err := regexp.Compile(expr)
				if err != nil {
					return fmt.Errorf("failed to compile include regex %q for backend %q: %w", expr, name, err)
				}
				tf.includeRegexps = append(tf.includeRegexps, re)
			}
		}
		newConfig.toolFilters[name] = tf
	}
...
func (m *MCPProxy) newSession(ctx context.Context, p *mcp.InitializeParams, route, subject string, span tracing.MCPSpan) (*session, error) {
	...
	backends := m.routes[route]
	if backends == nil {
		return nil, fmt.Errorf("no backends found for route %q", route)
	}
	entries = make([]compositeSessionEntry, len(backends))
	...
	for _, backend := range backends {
		...
		wg.Go(func() {
			...
			startAt := time.Now()
			initResult, err := m.initializeSession(ctx, backend, p)
			if err != nil {
	...
	encrypted, err := m.sessionCrypto.Encrypt(string(clientToGatewaySessionIDFromEntries(subject, finalEntries)))
	if err != nil {
...
func (m *MCPProxy) sessionFromID(id secureClientToGatewaySessionID, lastEvent secureClientToGatewayEventID) (*session, error) {
	...
	perBackendSessionIDs, err := clientToGatewaySessionID(decrypted).BackendSessionIDs()
	if err != nil {
		return nil, err
	}
	if len(lastEvent) != 0 {
	...
	return &session{id: id, proxy: m, perBackendSessions: perBackendSessionIDs}, nil
}
...
func (m *MCPProxy) initializeSession(ctx context.Context, backend filterapi.MCPBackend, p *mcp.InitializeParams) (*initializeResult, error) {
	...
	)
	{
		...
		resp, err := m.invokeJSONRPCRequest(ctx, backend, nil, mcpReq)
		if err != nil {
	...
	// Need to invoke "notifications/initialized" to complete the initialization.
	{
		...
		resp, err := m.invokeJSONRPCRequest(ctx, backend, &compositeSessionEntry{
...
func (m *MCPProxy) invokeJSONRPCRequest(ctx context.Context, backend filterapi.MCPBackend, cse *compositeSessionEntry, msg jsonrpc.Message) (*http.Response, error) {
	...
	addMCPHeaders(req, msg, backend.Name)
	if cse != nil {
