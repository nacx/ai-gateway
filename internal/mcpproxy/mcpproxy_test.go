...
`))
	...
	s, err := proxy.newSession(t.Context(), &mcp.InitializeParams{}, "test-route1", "", nil)
...
func TestInitializeSession_Success(t *testing.T) {
	...
	res, err := proxy.initializeSession(t.Context(), filterapi.MCPBackend{Name: "test-backend", Path: "/a/b/c"}, &mcp.InitializeParams{})
...
func TestInitializeSession_InitializeFailure(t *testing.T) {
	...
	sessionID, err := proxy.initializeSession(t.Context(), filterapi.MCPBackend{Name: "test-backend", Path: "/a/b/c"}, &mcp.InitializeParams{})
...
func TestInitializeSession_NotificationsInitializedFailure(t *testing.T) {
	...
	sessionID, err := proxy.initializeSession(t.Context(), filterapi.MCPBackend{Name: "test-backend", Path: "/aaaaaaaaaaaaaa"}, &mcp.InitializeParams{})
...
func TestInvokeJSONRPCRequest_Success(t *testing.T) {
	...
	resp, err := m.invokeJSONRPCRequest(t.Context(), filterapi.MCPBackend{Name: "test-backend", Path: "/aaaaaaaaaaaaaa"}, &compositeSessionEntry{
...
func TestInvokeJSONRPCRequest_NoSessionID(t *testing.T) {
	...
	m := newTestMCPProxy()
	m.backendListenerAddr = backendServer.URL
	resp, err := m.invokeJSONRPCRequest(t.Context(), filterapi.MCPBackend{Name: "test-backend", Path: "/mcp"}, &compositeSessionEntry{
		sessionID: "",
	}, &jsonrpc.Request{})
...
func TestToolFilter_Allows(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			filter: toolFilter{include: map[string]struct{}{"foo": {}}},
			tools:  []string{"foo", "bar"},
			want:   []bool{true, false},
		},
		{
			name:   "include regexp",
			filter: toolFilter{includeRegexps: []*regexp.Regexp{reBa}},
