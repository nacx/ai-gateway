...
func (s *session) Close() error {
	for backendName, sess := range s.perBackendSessions {
		...
		req, err := http.NewRequest(http.MethodDelete, s.proxy.mcpEndpointForBackend(s.proxy.backends[backendName]), nil)
		if err != nil {
		...
		addMCPHeaders(req, nil, backendName)
...
func (s *session) getSession(backend filterapi.MCPBackendName) *compositeSessionEntry {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *session) sendToAllBackends(ctx context.Context, httpMethod string, request *jsonrpc.Request, span tracing.MCPSpan) <-chan *sseEvent {
	...
	for backendName, cse := range s.perBackendSessions {
		...
		go func() {
			defer wg.Done()
			err := s.sendRequestPerBackend(ctx, backendMsgs, s.proxy.backends[backendName], cse, httpMethod, request)
			if err != nil {
...
func (s *session) sendRequestPerBackend(ctx context.Context, eventChan chan<- *sseEvent, backend filterapi.MCPBackend, cse *compositeSessionEntry,
	...
	addMCPHeaders(req, request, backend.Name)
...
type (
	...
	//
	//	{subject}@{mcp-backend-name1}:{base64(mcp-session-id1)},...,{mcp-backend-nameN}:{base64(mcp-session-idN)}
	//
	// For example:
	//
	//	"mcp-user@backend1:MTIzNDU2,backend2:NjU0MzIx"
	//
...
func (c clientToGatewaySessionID) BackendSessionIDs() (map[filterapi.MCPBackendName]*compositeSessionEntry, error) {
	perBackendSessionIDs := make(map[filterapi.MCPBackendName]*compositeSessionEntry)
	backendSessions := string(c[strings.LastIndex(string(c), "@")+1:]) // strip the subject part.
	parts := strings.Split(backendSessions, ",")
	for _, part := range parts {
		...
		if sessionIDBas64 != "" { // Some servers are stateless hence no (==empty) session ID.
			...
			if err != nil {
				...
				return perBackendSessionIDs, err
			}
	...
	return perBackendSessionIDs, nil
}
...
func clientToGatewaySessionIDFromEntries(subject string, entries []compositeSessionEntry) clientToGatewaySessionID {
	var b strings.Builder
	if subject != "" {
